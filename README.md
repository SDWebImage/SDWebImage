...
Open the "Build Settings" tab, in the "Linking" section, locate the "Other Linker Flags" setting and add the "-ObjC" flag (**Note:** it is capital `O` and capital `C`.  If it is not capital `O`, you will see `setImageWithURL:placeholderImage: unrecognized selector sent to instance` when you run the app)
